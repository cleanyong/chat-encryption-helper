...
/// 使用 HKDF-SHA256 建立簡單的 XOR stream cipher：
/// 每則訊息隨機產生 4-byte nonce，並用 (AES key, nonce) 經 HKDF 推導 keystream，再與明文 XOR。
/// 最終輸出 Base64( nonce || ciphertext )。
pub fn encrypt_chat_message(aes_key: &[u8; 32], plaintext: &str) -> Result<String, String> {
    let mut rng = OsRng;
    let nonce_u32 = rng.next_u32();
    let nonce = nonce_u32.to_be_bytes(); // 4 bytes

    let hk = Hkdf::<Sha256>::new(Some(&nonce), aes_key);
    let plaintext_bytes = plaintext.as_bytes();
    let mut keystream = vec![0u8; plaintext_bytes.len()];
    hk.expand(b"chat-message", &mut keystream)
        .map_err(|e| format!("HKDF expand for chat keystream failed: {e}"))?;
    ...
    let mut combined = Vec::with_capacity(4 + ciphertext.len());
    combined.extend_from_slice(&nonce);
    combined.extend_from_slice(&ciphertext);

    Ok(encode_b64(&combined))
}

/// 使用相同的 HKDF-XOR 流模式解密 Base64( nonce || ciphertext )。
pub fn decrypt_chat_message(aes_key: &[u8; 32], b64: &str) -> Result<String, String> {
    // 去掉所有空白字元，避免從聊天軟體貼上時夾雜空格或換行導致 padding 錯誤。
    let cleaned: String = b64.chars().filter(|c| !c.is_whitespace()).collect();
    if cleaned.is_empty() {
        return Err("ciphertext is empty".into());
    }

    let data = decode_b64(&cleaned).map_err(|e| format!("decode base64: {e}"))?;
    if data.len() < 5 {
        return Err("ciphertext too short".into());
    }
    let (nonce_bytes, ct) = data.split_at(4);
    let mut nonce = [0u8; 4];
    nonce.copy_from_slice(nonce_bytes);

    let hk = Hkdf::<Sha256>::new(Some(&nonce), aes_key);
    let mut keystream = vec![0u8; ct.len()];
    hk.expand(b"chat-message", &mut keystream)
        .map_err(|e| format!("HKDF expand for chat keystream failed: {e}"))?;

    let plaintext_bytes: Vec<u8> = ct
        .iter()
        .zip(keystream.iter())
        .map(|(c, k)| c ^ k)
        .collect();
